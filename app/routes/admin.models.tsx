...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === 'create') {
    ...
    if (!name || !model_id) {
      return Response.json({ error: 'Nombre y model_id son obligatorios' }, { status: 400, headers });
    }
  ...
  if (intent === 'update') {
    ...
    if (!id || !field) {
      return Response.json({ error: 'Faltan parámetros' }, { status: 400, headers });
    }
    ...
    if (field === 'is_active') {
      ...
      field === 'token_cost_multiplier' ||
      field === 'input_price_per_token' ||
      field === 'output_price_per_token'
    ) {
...
export default function AdminModelsRoute() {
  const { models } = useLoaderData<{ models: AIModel[] }>();
    ...
    <div className="min-h-screen bg-[#171717]">
      <div className="max-w-5xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-white">Gestión de Modelos de IA</h1>
        ...
        {showForm ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg bg-[#171717] border border-[#2F2F2F] px-3 py-2 text-white text-sm focus:border-[#9E7FFF] focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg bg-[#171717] border border-[#2F2F2F] px-3 py-2 text-white text-sm font-mono focus:border-[#9E7FFF] focus:outline-none"
                  />
                </div>
                <div>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#9E7FFF]/10 border border-[#9E7FFF]/30 text-[#9E7FFF] rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-[#9E7FFF]/20 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
