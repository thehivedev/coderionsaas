    ...
    <div className="min-h-screen overflow-hidden bg-[#090d18] text-white">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-48 border-r border-white/10 bg-[#0b0d13]/95 px-3 py-4 lg:block">
        ...
        <nav className="mt-5 space-y-1 text-xs">
          <div className="rounded-lg bg-white/10 px-3 py-2 font-medium text-white">Inicio</div>
          <Link to="/auth/register" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Proyectos</Link>
          <Link to="/auth/register" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Favoritos</Link>
          <Link to="/auth/register" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Vistos recientemente</Link>
        </nav>
        <div className="mt-7 border-t border-white/10 pt-5 text-xs">
          <p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-wider text-white/30">Recursos</p>
          <a href="#como-funciona" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Centro de ayuda</a>
          <a href="#caracteristicas" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Novedades</a>
          <a href="#precios" className="block rounded-lg px-3 py-2 text-white/55 transition-colors hover:bg-white/10 hover:text-white">Estado</a>
        </div>
        <div className="absolute bottom-4 left-3 right-3 rounded-xl border border-white/10 bg-gradient-to-br from-[#15284e] to-[#10131c] p-3">
          <p className="text-xs font-semibold text-white">Empieza gratis</p>
          <p className="mt-1 text-[10px] leading-4 text-white/45">Construye tu primera app con IA.</p>
          <Link to="/auth/register" className="mt-3 flex items-center justify-center rounded-lg bg-white py-2 text-[10px] font-semibold text-[#0757d9]">Crear cuenta</Link>
        </div>
      </aside>
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:pl-56 lg:pr-10">
        ...
        <nav className="hidden items-center gap-8 text-sm text-white/70 md:flex">
          ...
          <a href="#caracteristicas" className="transition-colors hover:text-white">Características</a>
      ...
      <main className="relative z-10">
        <section className="mx-auto flex min-h-[calc(100vh-80px)] w-full max-w-5xl flex-col items-center px-5 pb-20 pt-24 text-center sm:px-8 sm:pt-32 lg:pl-56 lg:pt-36">
          ...
          <h1 className="max-w-4xl text-5xl font-semibold leading-[1.02] tracking-[-0.055em] text-white sm:text-6xl lg:text-8xl">
            ¿Qué vas a
            <span className="block bg-gradient-to-r from-white via-[#b9eaff] to-[#54aaff] bg-clip-text text-transparent">crear hoy?</span>
          </h1>
          <p className="mt-6 max-w-xl text-base leading-7 text-white/65 sm:text-lg">
            Crea aplicaciones y sitios web increíbles conversando con IA. Describe tu idea y mira cómo cobra vida.
          </p>
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 px-2 pt-3">
              <div className="flex items-center gap-1.5">
                <button type="button" className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 text-white/60 transition-colors hover:bg-white/10 hover:text-white" aria-label="Añadir contexto">
                ...
                <span className="hidden text-xs text-white/40 sm:block">Empieza describiendo una idea</span>
              </div>
          ...
          <div className="mt-12 flex flex-wrap items-center justify-center gap-x-5 gap-y-3 text-xs text-white/45">
            ...
            <Link to="/auth/register" className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 transition-colors hover:bg-white/12 hover:text-white">
              <span className="font-semibold text-white/70">Una plantilla</span>
              <ArrowIcon />
            </Link>
          </div>
      ...
      <footer className="relative z-10 mx-auto flex max-w-7xl items-center justify-between border-t border-white/10 px-5 py-6 text-xs text-white/35 sm:px-8 lg:pl-56 lg:pr-10">
        ...
        <span>Construye algo extraordinario.</span>
      </footer>
    </div>
  );
}
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-y-auto">
        <div className="mx-auto w-full max-w-4xl px-6 py-12">
          ...
          <div className="mb-12 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-2xl bg-[#262626] p-5 ring-1 ring-[#2F2F2F]"><h2 className="mb-1 text-sm font-medium text-[#A3A3A3]">Tokens disponibles</h2><p className="text-2xl font-bold text-white">{data.profile.token_balance.toLocaleString()}</p></div>
            <div className="rounded-2xl bg-[#262626] p-5 ring-1 ring-[#2F2F2F]"><h2 className="mb-1 text-sm font-medium text-[#A3A3A3]">Modelo</h2><p className="text-2xl font-bold text-white">{data.defaultModelName}</p></div>
            <div className="rounded-2xl bg-[#262626] p-5 ring-1 ring-[#2F2F2F]"><h2 className="mb-1 text-sm font-medium text-[#A3A3A3]">Versión</h2><p className="text-2xl font-bold text-white">v{APP_VERSION}</p></div>
          </div>
          {data.recentProjects.length > 0 && <div><h3 className="mb-4 text-sm font-medium text-[#A3A3A3]">Proyectos recientes</h3><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{data.recentProjects.map((project) => <button key={project.id} onClick={() => navigate(`/project/${project.id}`)} className="group rounded-2xl bg-[#262626] p-4 text-left ring-1 ring-[#2F2F2F] transition-all hover:ring-[#9E7FFF]/30"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#9E7FFF]/10"><svg className="h-5 w-5 text-[#9E7FFF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-white transition-colors group-hover:text-[#9E7FFF]">{project.title}</p><p className="text-xs text-[#A3A3A3]">{new Date(project.updated_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' })}</p></div></div></button>)}</div></div>}
        </div>
      </main>
    </div>
  );
